...
`docker compose up` smoke test → deploy.
...
Gates, all currently passing: **217 tests** (including the API routes and the
schema migration), `ruff` clean, `mypy --strict` clean across 34 modules,
`tsc --noEmit` clean.
